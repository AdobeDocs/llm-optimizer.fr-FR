...
L’intégration Adobe Analytics connecte LLM Optimizer aux données Adobe Analytics de votre entreprise, afin que vous puissiez mesurer la manière dont les découvertes pilotées par l’IA se traduisent par un engagement réel sur le site web et par des résultats commerciaux. Une fois le processus d’intégration terminé, les données sont disponibles dans le tableau de bord **Trafic de recommandation** sous l’onglet **Impact commercial**.

En liant les données d’analyse aux informations de visibilité de l’IA, LLM Optimizer vous permet de suivre les informations suivantes :

* Interaction de la clientèle sur les pages recommandées par l’IA.
* Signaux de conversion liés aux parcours de découverte de l’IA.
* Impact des optimisations GEO sur les performances.

Cette intégration permet de combler le fossé entre la mesure de visibilité de l’IA et l’analyse des performances commerciales. Les équipes peuvent désormais voir non seulement où la marque apparaît dans les réponses de l’IA, mais aussi ce qui se passe après l’arrivée des utilisateurs et utilisatrices.

## Disponibilité {#availability}

>[!IMPORTANT]
>
>L’intégration d’Adobe Analytics est incluse dans l’offre LLM Optimizer payante. La clientèle en version d’essai gratuite ne pourra pas connecter Adobe Analytics tant qu’elle n’aura pas effectué la mise à niveau vers une offre payante.
...
1. Ouvrez le tableau de bord du [Trafic de recommandation](/help/dashboards/referral-traffic.md). La vue par défaut est **Informations sur le trafic**.
   ...
   ![Tableau de bord du Trafic de recommandation](/help/dashboards/assets/aa-integration-01-referral-traffic-traffic-insights.png)
...
1. Cliquez sur **Se connecter à Analytics**. Le tableau de bord de [configuration cliente](/help/dashboards/customer-configuration.md) s’ouvre alors dans l’onglet **Analytics**.

   ![Configuration cliente, onglet Analytics](/help/dashboards/assets/aa-integration-03-analytics-tab.png)
...
1. Sous **Informations d’identification**, saisissez l’**ID client** et le **Secret client**, puis sélectionnez **Vérifier et continuer**. Veuillez noter les points suivants :
   ...
   * Utilisez l’**ID client** et le **Secret client** d’un [compte technique](https://developer.adobe.com/developer-console/docs/guides/authentication/ServerToServerAuthentication/) qui a accès à la suite de rapports dont vous avez besoin.
   ...
   ![Informations d’identification Analytics avec option Vérifier et continuer](/help/dashboards/assets/aa-integration-04-credentials.png)
...
1. Choisissez une **Dimension d’URL de page** (liste de dimensions spécifique à la suite), puis sélectionnez **Enregistrer et activer**.

   * La **Dimension d’URL de page** reste désactivée jusqu’à ce qu’une suite de rapports soit sélectionnée et que les dimensions soient chargées.
   * L’option **Enregistrer et activer** n’est disponible qu’après avoir sélectionné une dimension d’URL de page.

   ![Dimension URL de la page avec option Enregistrer et activer](/help/dashboards/assets/aa-integration-06-page-url-dimension.png)

1. Après l’enregistrement, la configuration doit afficher le statut **Connecté**. Vous pouvez revenir au tableau de bord du Trafic de recommandation avec **Accéder au tableau de bord Trafic de recommandation**. Dans **Trafic de recommandation** dans l’onglet **Impact commercial**, le statut doit apparaître comme **Connecté à Adobe Analytics**.

   ![Connecté à Adobe Analytics, dans la configuration et Impact commercial](/help/dashboards/assets/aa-integration-07-connected.png)
...
## Voir Impact commercial en action

La visibilité de l&#39;IA n&#39;est qu&#39;une partie de l&#39;histoire. Pour comprendre si vos efforts d’optimisation génèrent des résultats commerciaux, vous devez savoir ce qui se passe une fois que les visiteurs et visiteuses arrivent sur votre site.
...
Le trafic provenant de LLM est identifié à l’aide d’Adobe Analytics [Type de référent : outils d’IA dédié à la conversation](https://experienceleague.adobe.com/fr/docs/analytics/components/dimensions/referrer-type#conversational-ai-tools).
...
Ce jeu de données alimente les informations LLM Optimizer pour les données suivantes :
...
Q : L’intégration d’Adobe Analytics est-elle disponible pendant la période d’essai ?
...
Les URL complètes utilisées pour la dimension de page sélectionnée peuvent être ingérées ; les chaînes de requête et le contenu des pages ne sont pas ingérés pour cette intégration.

Q : Les identifiants d’utilisation (ECID, adresse IP, ID de cookie) sont-ils stockés ?
